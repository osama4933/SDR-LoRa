...
FS = 500e3  # 500000  # recerver's sampling rate
...
def act_and_send(Pkt_To_Send, queue_size):
    ...
    # for SF in LORA_SF:
    #     pkts = demoder.Evaluate(buffer, SF, LORA_BW, FS, True)
    # create active session object
    if not isinstance(buffer, numpy.ndarray):
        return
    BW = 5e6
    sz_ap = BW / 8#len(LORA_CHANNELS)

    def_sz = len(buffer) * 2 * 32  # bits
    # print(f'Size of buffer in bits = {sz} & # of chans = {len(LORA_CHANNELS)} & sz per ap req is = {sz_ap}')

    sz = sz_ap + 1
    i = -1
    while sz > sz_ap and i < 8:
        i = i + 1
        action = dden * i * 5
        zz = Active_Session(ID, channel, pkt_counter, start_t, action, buffer, info[2], SEND_SIZE, FS)
        file = zz.build_packet()
        sz = len(file) * 8
        # print(f'size of pkt = {sz} vs ap_sz = {sz_ap} & i = {i}')

    # print('After Compression\n')
    msglen = int.from_bytes(file[:4], 'big')
    z = lz4.frame.decompress(file[4:msglen + 4])  # unzip file
    pkt = Active_Session.from_packet(z)
    demoder = Std_LoRa(NUM_PREAMBLE, NUM_SYNC, NUM_DC, MAX_DATA_SYM, HAS_CRC)
    for SF in LORA_SF:
        # print(f'decoding SF = {SF}')
        pkts = demoder.Evaluate(pkt.buffer, SF, LORA_BW, FS, True)
        for p in pkts:
            # print(f'{p}')
            f1 = open('RLO_out.txt', 'a')
            f1.write(f'{int(channel)},')
            f1.write(f'{int(i)},')
            f1.write(f'{int(sz)},')
            f1.write(f'{int(def_sz)},')
            f1.write(f'{p[2].tolist()}\n')
            f1.close()
    # if len(pkts) != 0:
    #     print(pkts)
...
if __name__ == "__main__":
    ...
    # c.start()
    # d.start()
    # multiprocessing.Process(target=uplink_manager, args=([])).start()
    myPool = multiprocessing.Pool(1, uplink_manager, ([]))

    # myPool = multiprocessing.Pool(1, comp_send, (comp_q, out_q))
    # myPool = multiprocessing.Pool(2, uplink_manager, ())
